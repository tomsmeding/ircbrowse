...
versions = zip [1..] ms where
  ms = [ex ["CREATE TABLE event"
       ...
       , do ex ["create table network (name text not null, title text not null)"]
        ...
        ,do ex ["create table conversation_by_year (year int not null, lines int not null, channel int not null, unique (channel, year));"]
            ...
            ex ["create table general_activity_by_year (year int not null, lines int not null, channel int not null, unique (channel, year));"]
